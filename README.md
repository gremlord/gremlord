...
- **Fidelity.** Non-Anthropic models work through translation, but Claude Code's prompts and tool patterns are tuned for Claude, so expect them to be clunkier in the main loop. They shine as cheap workhorses for background tasks and subagents. Specific gaps: no `cache_control` breakpoints on OpenAI-dialect backends (provider-side implicit caching still shows up as cache reads, measurable with `gremlord cost`), thinking is display-only on Chat Completions, while Responses replays encrypted reasoning and assistant phases through a bounded session cache (see [GPT fidelity and context handling](docs/gpt-fidelity.md)), Anthropic server tools (web search, code execution) are unavailable on translated models, `top_k` is dropped, stop sequences truncate to four (and are dropped entirely on the Responses flavor), and token counting for translated models is a deliberate ~15% overestimate so auto-compact fires early instead of overflowing context. OpenAI-compatible providers default to `/v1/chat/completions`; set `api: responses` on a provider or model (`gremlord models add … --api responses`) for OpenAI's `/v1/responses` — required for models like gpt-6-astra that reject function tools combined with `reasoning_effort` on chat completions. Responses also preserves parallel calls, optional tool arguments, and uses measured prior usage to account for hidden reasoning in the context gauge. Set `reasoning_effort` on a model (or `--reasoning-effort` when adding it) to pin a supported effort alongside `reasoning: effort`; otherwise explicit client effort takes precedence over the legacy thinking budget. xAI, vLLM, and other compatible endpoints stay on chat completions unless they actually implement Responses. Set `max_output` on models whose output cap is below what Claude Code requests (it asks for 32K), and `context_window` on models whose window differs from the ~200K Claude Code assumes (see [Context scaling](#context-scaling)).
